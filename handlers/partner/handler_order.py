...
from filter.user_filter import IsRoleAdmin, IsRolePartner
...
@router.message(F.text == 'Заказы', or_f(IsRolePartner(), IsRoleAdmin()))
@error_handler
async def press_button_order(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    :return:
    """
    logging.info(f'press_button_order: {message.from_user.id}')
    await message.answer(text='Выберите раздел',
                         reply_markup=kb.keyboard_action_order())
